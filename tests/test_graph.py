...
def idents():
    return [
        ...
        Identity("2", "mlp-prod", "AWSAdministratorAccess"),
    ]
...
def test_access_surface_allow_and_deny(tmp_path):
    ...
    assert by[("mlp-dev", "AWSReadOnlyAccess")].allowed is True
    # admin is not read-only and matches no allow -> agent default deny
    assert by[("mlp-dev", "AWSAdministratorAccess")].allowed is False
    # prod admin hit the explicit deny rule
    assert by[("mlp-prod", "AWSAdministratorAccess")].allowed is False
    assert "deny" in by[("mlp-prod", "AWSAdministratorAccess")].reason.lower()
...
def test_render_access_surface_is_self_contained(tmp_path):
    ...
    assert "mlp-dev" in html and "AWSReadOnlyAccess" in html
