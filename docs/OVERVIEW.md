...
- **The endpoint door** (a later phase) is for any AWS SDK or tool like
  Terraform. grantry pretends to be the small credential service that AWS SDKs
  already know how to read, so those tools get credentials with no config file
  at all.
...
agents:
  allow:
    - identity: "*/ReadOnlyAccess"          # any account, read only role
    - identity: "dev-*/AWSPowerUserAccess"  # power user, but only in dev accounts
  deny:
    - identity: "*prod*/*Admin*"            # never admin in a prod account
...
your rule says fifteen minutes, it gets fifteen. If the policy file is missing
or broken, agents get nothing (humans still work), so a mistake fails safe.

An identity is written as `account-name/role-name`, and you can use `*` as a
wildcard, so `dev-*/ReadOnlyAccess` means "read only in any account whose name
starts with dev".
...
- **`grantry login`**
  Logs you into AWS Identity Center. It prints a link and a code; you approve
  in your browser once, and grantry remembers the session (in the keychain).
  You log in to the whole Identity Center at once, not to a single account.

- **`grantry ls`**
  Lists every account and role you can use, one `account/role` per line.

- **`grantry audit`**
  Prints the grant history in plain lines.

- **`grantry mcp`**
  Starts grantry as an MCP server so an agent can call it. You point your
  agent's MCP config at this command once. It exposes these tools to the agent:
  ...
  - `check_access(identity)` lets the agent ask "would this be allowed?"
    before it tries, so it can plan instead of failing.

### Phase 2 (next plan: finishing the human side)

- **`grantry switch <identity>`**
  Drops you into a shell (or prints export lines) already set up as that
  account and role, so every command you run uses it.

- **`grantry run <identity> -- <command>`**
  Runs a single command as a chosen identity without changing anything else,
  for example `grantry run prod/ReadOnlyAccess -- aws s3 ls`.

- **`grantry populate`**
  Writes profiles into your `~/.aws/config` for every account and role you can
  use, so the normal `aws` CLI keeps working. It is tidy: it adds new ones,
  updates changed ones, and removes stale ones it created before. A
...
- **`grantry serve --container-endpoint`**
  Turns on the endpoint door. After this, tools like Terraform, boto3, and the
  Go and Java SDKs pick up credentials automatically from grantry with a single
  environment variable and no config file at all.

- **`request_login` (an MCP tool, added in this phase)**
  When an agent needs credentials but no one is logged in, it calls this.
  grantry pops a desktop notification and waits. You approve the login once,
  and the agent's original request continues on its own. This is how an agent
  recovers from an expired session without a human babysitting it.

### Phase 3 (later: more clouds and teams)

- **Azure and GCP providers**
  The same commands (`login`, `ls`, `switch`, `run`), now able to say
  `grantry login --cloud azure`. Only a new provider file is added; the engine,
  policy, audit, and MCP doors are unchanged.

- **Team mode**
  A way to share one signed policy file across a team, so a lead sets the rules
  once and everyone's grantry enforces them.

- **runclave integration**
  grantry becomes the piece that safely injects credentials into runclave's
